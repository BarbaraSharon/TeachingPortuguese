...
function FeatureCard({item, iconSvg, imgData, variant = "grid", large = false}) {
  ...
  const linkedCls = item.url
    ? "block cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-4"
    : "";
  ...
  if (item.url) {
    return (
      <a href={item.url} class={`${wrapperCls} ${linkedCls}`} aria-label={item.name || "View learning path"}>
        {cardContent}
      </a>
    );
  }

  return <div class={wrapperCls}>{cardContent}</div>;
}
