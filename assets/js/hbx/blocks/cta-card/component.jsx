...
function getAction(content) {
  return content?.button || content?.primary_action || {};
}

export const CtaCardBlock = ({content, design, _id, button_icon_svg, icon_svg}) => {
  ...
  const button = getAction(content);
  ...
  const CardTag = buttonUrl ? "a" : "div";
  const resolvedIcon = button_icon_svg || icon_svg;

  return (
    <CardTag
      href={buttonUrl || undefined}
      target={buttonUrl && isNewTab ? "_blank" : undefined}
      rel={buttonUrl && isNewTab ? "noopener" : undefined}
      aria-label={buttonUrl && buttonText ? buttonText : undefined}
      class={`group relative overflow-hidden ${finalCardClass} p-8 sm:p-12 lg:p-16 xl:p-20 mx-auto max-w-6xl rounded-3xl shadow-2xl flex flex-col items-center text-center ${buttonUrl ? "cursor-pointer hover:shadow-3xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2" : ""}`}
      style={style}
    >
      {content.title && (
        <h2
      ...
      {showButton && (
        <span
          class={`mt-10 inline-flex items-center gap-3 rounded-2xl px-8 py-4 text-lg font-semibold ${buttonBgClasses} ${buttonTextClasses} transition-all duration-300 ease-out group-hover:scale-105 group-hover:shadow-2xl`}
        >
          <span>{buttonText}</span>
          {resolvedIcon && (
            <span class="transition-transform duration-300 group-hover:translate-x-1">
              <Icon svg={resolvedIcon} attributes={{style: "height: 1.25em", class: "inline-block"}} />
            </span>
          )}
        </span>
      )}
